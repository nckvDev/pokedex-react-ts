...
export const Slide = ({ children }: SlideProps) => {
  ...
  return (
    <C.SlideContainer>
      {!mobile ? (
        <C.SlideButton value='prev' onClick={slideNavigation} disabled={slidePosition === 0 ? true : false}>
          <LeftArrowIcon />
        </C.SlideButton>
      ) : null}
      <C.Slide>
        <C.SlideContent slidePosition={slidePosition}>{children}</C.SlideContent>
      </C.Slide>
      {!mobile ? (
        <C.SlideButton value='next' onClick={slideNavigation} disabled={slidePosition === -87.5 ? true : false}>
          <RightArrowIcon />
        </C.SlideButton>
      ) : null}
    </C.SlideContainer>
